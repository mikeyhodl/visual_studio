...
Public Class frmStatisticalAnalysis
    ...
    Private Sub frmStatisticalAnalysis_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Try
            ...
            dtadptTotalSchoolEnrolment = New OleDb.OleDbDataAdapter("SELECT COUNT(ADMISSIONNUMBER) as TotalSchoolEnrolment FROM tblNEWSTUDENTS WHERE DISCONTINUED = FALSE", conn) 'WHERE ADMISSIONNUMBER IN (SELECT DISTINCT ADMISSIONNUMBER FROM tblSTUDENTSNIDHAMU)", conn)


            ' Use the data adapter to fill data in the dataset
            dtadptTotalSchoolEnrolment.Fill(dstTotalSchoolEnrolment, "tblNEWSTUDENTS")
            ...
            dtadptTotalNumberOfOffenses = New OleDb.OleDbDataAdapter("SELECT COUNT(OFFENSECATEGORY) as TotalOffenses FROM tblSTUDENTSNIDHAMU WHERE OFFENSECATEGORY <> 'DISCIPLINED'", conn)


            ' Use the data adapter to fill data in the dataset
            dtadptTotalNumberOfOffenses.Fill(dstTotalNumberOfOffenses, "tblSTUDENTSNIDHAMU")
            ...
            Me.TotalSchoolPopulation.Text = dstTotalSchoolEnrolment.Tables("tblNEWSTUDENTS").Rows(0).Item("TotalSchoolEnrolment")
            Me.TotalNumberOfOffenses.Text = dstTotalNumberOfOffenses.Tables("tblSTUDENTSNIDHAMU").Rows(0).Item("TotalOffenses")
            ...
            'fetch data for the combobox 'CombClass'

            Dim dstClass As New DataSet()
            Dim dtadptClass As OleDbDataAdapter
            ...
            'Set the data adapter and fetch data from the database using select query
            dtadptClass = New OleDb.OleDbDataAdapter("SELECT DISTINCT CLASS From tblSTUDENTSNIDHAMU", conn)

            ' Use the data adapter to fill data in the dataset
            dtadptClass.Fill(dstClass, "tblSTUDENTSNIDHAMU") 'For combobox CombClass

            conn.Close()

            'Fill the combobox 'CombClass'

            CombClass.DataSource = dstClass.Tables("tblSTUDENTSNIDHAMU")
            CombClass.DisplayMember = "CLASS"
            CombClass.ValueMember = "CLASS"



            'Fill the combobox 'CombClass2'

            CombClass2.DataSource = dstClass.Tables("tblSTUDENTSNIDHAMU")
            CombClass2.DisplayMember = "CLASS"
            CombClass2.ValueMember = "CLASS"
            ...
            'fetch data for the combobox 'CombStream'

            Dim dstStream As New DataSet()
            Dim dtadptStream As OleDbDataAdapter


            'clear the previous record before fetching new record and bind it to the combobox 'CombStream'
            dstStream.Clear()


            'Set the data adapter and fetch data from the database using select query
            dtadptStream = New OleDb.OleDbDataAdapter("SELECT DISTINCT STREAM From tblSTUDENTSNIDHAMU", conn)

            ' Use the data adapter to fill data in the dataset
            dtadptStream.Fill(dstStream, "tblSTUDENTSNIDHAMU") 'For combobox CombStream

            conn.Close()

            'Fill the combobox 'CombStream'

            CombStream.DataSource = dstStream.Tables("tblSTUDENTSNIDHAMU")
            CombStream.DisplayMember = "STREAM"
            CombStream.ValueMember = "STREAM"



            'Fill the combobox 'CombStream2'

            CombStream2.DataSource = dstStream.Tables("tblSTUDENTSNIDHAMU")
            CombStream2.DisplayMember = "STREAM"
            CombStream2.ValueMember = "STREAM"
    ...
    Private Sub BtnAnalysisByOffenseType_Click(sender As Object, e As EventArgs) Handles BtnAnalysisByOffenseType.Click
        ...
        Try
            ...
            'Set the data adapter and fetch data from the database using select query
            dtadptAnalysisByOffenseType = New OleDb.OleDbDataAdapter("SELECT DATEOFOCCURENCE, OFFENSECATEGORY, COUNT(OFFENSECATEGORY) as COUNTOFOFFENSESCOMMITED, COUNT(OFFENSECATEGORY)/ '" & Me.TotalSchoolPopulation.Text & "' *100 as PERCENTAGE  From tblSTUDENTSNIDHAMU WHERE DATEOFOCCURENCE >= #" & Format(DtpBeginningDate.Value, "dd/MMM/yyyy") & "# AND DATEOFOCCURENCE <= #" & Format(DtpEndingDate.Value, "dd/MMM/yyyy") & "# AND OFFENSECATEGORY <> 'DISCIPLINED' GROUP BY DATEOFOCCURENCE, OFFENSECATEGORY ORDER BY DATEOFOCCURENCE DESC", conn)

            dtadptAnalysisByOffenseType.Fill(dstAnalysisByOffenseType, "tblSTUDENTSNIDHAMU")
            ' Use the data adapter to fill data in the dataset
            ...
            If strCount <> -1 Then
                ...
                If dstAnalysisByOffenseType.Tables("tblSTUDENTSNIDHAMU").Rows.Count > 0 Then

                    For strCount = 0 To dstAnalysisByOffenseType.Tables("tblSTUDENTSNIDHAMU").Rows.Count - 1

                        dstAnalysisNewRow = dstOffenseCategoryAnalysis.Tables("tblSTUDENTSOFFENSECATEGORYANALYSIS").NewRow()
                        dstAnalysisNewRow.Item("DATEOFOCCURENCE") = dstAnalysisByOffenseType.Tables("tblSTUDENTSNIDHAMU").Rows(strCount).Item("DATEOFOCCURENCE")
                        dstAnalysisNewRow.Item("OFFENSECATEGORY") = dstAnalysisByOffenseType.Tables("tblSTUDENTSNIDHAMU").Rows(strCount).Item("OFFENSECATEGORY").ToString
                        dstAnalysisNewRow.Item("OCCURENCECOUNT") = dstAnalysisByOffenseType.Tables("tblSTUDENTSNIDHAMU").Rows(strCount).Item("COUNTOFOFFENSESCOMMITED")
            ...
            DGVStatisticalAnalysis.DataSource = dstAnalysisByOffenseType.Tables("tblSTUDENTSNIDHAMU")
    ...
    Private Sub BtnOverallOffenseAnalysis_Click(sender As Object, e As EventArgs) Handles BtnOverallOffenseAnalysis.Click
        ...
        Try
            ...
            dtadptOverallOffenseAnalysis = New OleDb.OleDbDataAdapter("SELECT COUNT(ADMISSIONNUMBER) as OVERALLOFFENSESCOUNT, COUNT(ADMISSIONNUMBER)/ '" & TotalSchoolPopulation.Text & "' *100 as PERCENTAGE FROM tblNEWSTUDENTS WHERE DISCONTINUED = FALSE AND ADMISSIONNUMBER IN (SELECT DISTINCT ADMISSIONNUMBER FROM tblSTUDENTSNIDHAMU WHERE DATEOFOCCURENCE >= #" & Format(DtpBeginningDate.Value, "dd/MMM/yyyy") & "# AND DATEOFOCCURENCE <= #" & Format(DtpEndingDate.Value, "dd/MMM/yyyy") & "# AND OFFENSECATEGORY <> 'DISCIPLINED')", conn) 'AND OFFENSECATEGORY <> NULL)", conn)


            ' Use the data adapter to fill data in the dataset
            dtadptOverallOffenseAnalysis.Fill(dstOverallOffenseAnalysis, "tblNEWSTUDENTS")
            ...
            DGVStatisticalAnalysis.DataSource = dstOverallOffenseAnalysis.Tables("tblNEWSTUDENTS")
    ...
    Private Sub BtnGenerateOffenseCategoriesChart_Click(sender As Object, e As EventArgs) Handles BtnGenerateOffenseCategoriesChart.Click
        ...
        Try
            ...
            strSQL = "SELECT OFFENSECATEGORY, COUNT(OFFENSECATEGORY) as OCCURENCECOUNT From tblSTUDENTSNIDHAMU WHERE DATEOFOCCURENCE >= #" & Format(DtpBeginningDate.Value, "dd/MMM/yyyy") & "# AND DATEOFOCCURENCE <= #" & Format(DtpEndingDate.Value, "dd/MMM/yyyy") & "# AND ACADEMICYEAR = '" & txtCurrentYear.Text & "' AND OFFENSECATEGORY <> 'DISCIPLINED' AND ADMISSIONNUMBER IN (SELECT DISTINCT ADMISSIONNUMBER FROM tblNEWSTUDENTS WHERE DISCONTINUED = FALSE) GROUP BY OFFENSECATEGORY"
    ...
    Private Sub BtnOverallIndisciplinePercentages_Click(sender As Object, e As EventArgs) Handles BtnOverallIndisciplinePercentages.Click
        ...
        Try
            ...
            strSQL = "SELECT COUNT(ADMISSIONNUMBER) as NumberOfIndisciplined, COUNT(ADMISSIONNUMBER)/ '" & TotalSchoolPopulation.Text & "' as PercentageOfIndisciplined FROM tblNEWSTUDENTS WHERE DISCONTINUED = FALSE AND ADMISSIONNUMBER IN (SELECT DISTINCT ADMISSIONNUMBER FROM tblSTUDENTSNIDHAMU WHERE DATEOFOCCURENCE >= #" & Format(DtpBeginningDate.Value, "dd/MMM/yyyy") & "# AND DATEOFOCCURENCE <= #" & Format(DtpEndingDate.Value, "dd/MMM/yyyy") & "# AND ACADEMICYEAR = '" & txtCurrentYear.Text & "' AND OFFENSECATEGORY <> 'DISCIPLINED')"


            'SELECT OFFENSECATEGORY, COUNT(OFFENSECATEGORY) as OCCURENCECOUNT, COUNT(OFFENSECATEGORY)/ '" & Me.TotalSchoolPopulation.Text & "' *100 as PERCENTAGEOFOCCURENCE  From tblSTUDENTSNIDHAMU WHERE DATEOFOCCURENCE >= #" & Format(DtpBeginningDate.Value, "dd/MMM/yyyy") & "# AND DATEOFOCCURENCE <= #" & Format(DtpEndingDate.Value, "dd/MMM/yyyy") & "# AND OFFENSECATEGORY <> 'NONE' GROUP BY OFFENSECATEGORY ORDER BY OFFENSECATEGORY DESC"
    ...
    Private Sub BtnGenerateIndisciplineCases2report_Click(sender As Object, e As EventArgs) Handles BtnGenerateIndisciplineCases2report.Click
        ...
        Try
            ...
            strSQL &= "TYPEOFOFFENSE, OFFENSECATEGORY, FAMILYSITUATION, NAMEOFPARENT,  PARENTPHONENUMBER, TYPEOFPUNISHMENT, REMARKS FROM tblSTUDENTSNIDHAMU WHERE DATEOFOCCURENCE >= #" & Format(DtpBeginningDate.Value, "dd/MMM/yyyy") & "# AND DATEOFOCCURENCE <= #" & Format(DtpEndingDate.Value, "dd/MMM/yyyy") & "# AND ACADEMICYEAR = '" & txtCurrentYear.Text & "' AND OFFENSECATEGORY <> 'DISCIPLINED' AND ADMISSIONNUMBER IN (SELECT DISTINCT ADMISSIONNUMBER FROM tblNEWSTUDENTS WHERE DISCONTINUED = FALSE) ORDER BY DATEOFOCCURENCE ASC"
    ...
    Private Sub BtnViewLeavers_Click(sender As Object, e As EventArgs) Handles BtnViewLeavers.Click
        ...
        Try
            ...
            strSQL &= "TYPEOFOFFENSE, OFFENSECATEGORY, FAMILYSITUATION, NAMEOFPARENT,  PARENTPHONENUMBER FROM tblSTUDENTSNIDHAMU WHERE DATEOFOCCURENCE >= #" & Format(DtpBeginningDate.Value, "dd/MMM/yyyy") & "# AND DATEOFOCCURENCE <= #" & Format(DtpEndingDate.Value, "dd/MMM/yyyy") & "# AND OFFENSECATEGORY <> 'DISCIPLINED' AND ADMISSIONNUMBER IN (SELECT DISTINCT ADMISSIONNUMBER FROM tblNEWSTUDENTS WHERE DISCONTINUED = TRUE) ORDER BY DATEOFOCCURENCE ASC"
    ...
    Private Sub BtnIndisciplinedStudentsByClass_Click(sender As Object, e As EventArgs) Handles BtnIndisciplinedStudentsByClass.Click
        ...
        Try
            ...
            strSQL &= "TYPEOFOFFENSE, OFFENSECATEGORY, FAMILYSITUATION, NAMEOFPARENT,  PARENTPHONENUMBER, TYPEOFPUNISHMENT FROM tblSTUDENTSNIDHAMU WHERE DATEOFOCCURENCE >= #" & Format(DtpBeginningDate.Value, "dd/MMM/yyyy") & "# AND DATEOFOCCURENCE <= #" & Format(DtpEndingDate.Value, "dd/MMM/yyyy") & "# AND ACADEMICYEAR = '" & txtCurrentYear.Text & "' AND OFFENSECATEGORY <> 'DISCIPLINED' AND ADMISSIONNUMBER IN (SELECT DISTINCT ADMISSIONNUMBER FROM tblNEWSTUDENTS WHERE DISCONTINUED = FALSE) ORDER BY DATEOFOCCURENCE ASC"
    ...
    Private Sub BtnGenderOffenceAnalysis_Click(sender As Object, e As EventArgs) Handles BtnGenderOffenceAnalysis.Click
        ...
        Try
            ...
            strSQL = "SELECT GENDER, TYPEOFOFFENSE, OFFENSECATEGORY, COUNT(OFFENSECATEGORY) as OCCURENCECOUNT, COUNT(ADMISSIONNUMBER) as GENDERENROLMENT From tblSTUDENTSNIDHAMU WHERE DATEOFOCCURENCE >= #" & Format(DtpBeginningDate.Value, "dd/MMM/yyyy") & "# AND DATEOFOCCURENCE <= #" & Format(DtpEndingDate.Value, "dd/MMM/yyyy") & "# AND ADMISSIONNUMBER IN (SELECT DISTINCT ADMISSIONNUMBER FROM tblNEWSTUDENTS WHERE DISCONTINUED = FALSE) GROUP BY GENDER, TYPEOFOFFENSE, OFFENSECATEGORY ORDER BY GENDER ASC"
            ...
            ' strSQL = "SELECT GENDER, COUNT(ADMISSIONNUMBER) as NumberOfIndisciplined, COUNT(ADMISSIONNUMBER)/ 499 as PercentageOfIndisciplined FROM tblNEWSTUDENTS WHERE DISCONTINUED = FALSE AND ADMISSIONNUMBER IN (SELECT DISTINCT ADMISSIONNUMBER FROM tblSTUDENTSNIDHAMU WHERE DATEOFOCCURENCE >= #" & Format(DtpBeginningDate.Value, "dd/MMM/yyyy") & "# AND DATEOFOCCURENCE <= #" & Format(DtpEndingDate.Value, "dd/MMM/yyyy") & "# AND OFFENSECATEGORY <> 'DISCIPLINED') GROUP BY GENDER ORDER BY GENDER ASC"
    ...
    Private Sub BtnIndividualMwanafunzi_Click(sender As Object, e As EventArgs) Handles BtnIndividualMwanafunzi.Click
        ...
        Try
            ...
            strSQL &= "TYPEOFOFFENSE, OFFENSECATEGORY, FAMILYSITUATION, NAMEOFPARENT,  PARENTPHONENUMBER, STUDENTPHOTO, COUNTY, SUBCOUNTY, RESIDENCEESTATE, RESPONSIBILITY, REMARKS, TYPEOFPUNISHMENT, FEESBALANCE FROM tblSTUDENTSNIDHAMU WHERE DATEOFOCCURENCE >= #" & Format(DtpBeginningDate.Value, "dd/MMM/yyyy") & "# AND DATEOFOCCURENCE <= #" & Format(DtpEndingDate.Value, "dd/MMM/yyyy") & "# AND ADMISSIONNUMBER = '" & ComboADMNO.Text & "' AND ADMISSIONNUMBER IN (SELECT DISTINCT ADMISSIONNUMBER FROM tblNEWSTUDENTS) ORDER BY DATEOFOCCURENCE ASC"



            'tblSTUDENTSNIDHAMU.TYPEOFOFFENSE, tblSTUDENTSNIDHAMU.OFFENSECATEGORY, 


            ' strSQL = " SELECT tblSTUDENTSNIDHAMU.ID, tblSTUDENTSNIDHAMU.DATEOFOCCURENCE, tblSTUDENTSNIDHAMU.ADMISSIONNUMBER, tblSTUDENTSNIDHAMU.TERM, "

            ' strSQL &= "tblSTUDENTSNIDHAMU.NAMEOFSTUDENT, tblSTUDENTSNIDHAMU.GENDER, tblSTUDENTSNIDHAMU.KCPEMARKS, tblSTUDENTSNIDHAMU.KCPEGRADE, tblSTUDENTSNIDHAMU.CURRENTMARKS, tblSTUDENTSNIDHAMU.CURRENTPOINTS, "

            ' strSQL &= "tblSTUDENTSNIDHAMU.CURRENTGRADE, tblSTUDENTSNIDHAMU.STUDENTVAP, tblSTUDENTSNIDHAMU.CLASS, tblSTUDENTSNIDHAMU.STREAM, tblSTUDENTSNIDHAMU.FORMERSCHOOL, tblSTUDENTSNIDHAMU.DATEOFADMISSION, tblSTUDENTSNIDHAMU.DORMITORIES, "

            ' strSQL &= "tblSTUDENTSNIDHAMU.FAMILYSITUATION, tblSTUDENTSNIDHAMU.NAMEOFPARENT, tblSTUDENTSNIDHAMU.PARENTPHONENUMBER, tblSTUDENTSNIDHAMU.STUDENTPHOTO, tblSTUDENTSNIDHAMU.COUNTY, "

            ' strSQL &= "tblSTUDENTSNIDHAMU.SUBCOUNTY, tblSTUDENTSNIDHAMU.RESIDENCEESTATE, tblSTUDENTSNIDHAMU.RESPONSIBILITY, tblSTUDENTSNIDHAMU.REMARKS, tblSTUDENTSNIDHAMU.TYPEOFPUNISHMENT, tblSTUDENTSNIDHAMU.FEESBALANCE, tblSTUDENTSNIDHAMUArchive.ID, tblSTUDENTSNIDHAMUArchive.DATEOFOCCURENCE, tblSTUDENTSNIDHAMUArchive.TYPEOFOFFENSE, tblSTUDENTSNIDHAMUArchive.OFFENSECATEGORY FROM tblSTUDENTSNIDHAMU INNER JOIN tblSTUDENTSNIDHAMUArchive ON tblSTUDENTSNIDHAMU.ADMISSIONNUMBER = tblSTUDENTSNIDHAMUArchive.ADMISSIONNUMBER WHERE tblSTUDENTSNIDHAMU.ADMISSIONNUMBER = '" & ComboADMNO.Text & "'" 'AND tblSTUDENTSNIDHAMU.DATEOFOCCURENCE >= #" & Format(DtpBeginningDate.Value, "dd/MMM/yyyy") & "# AND tblSTUDENTSNIDHAMU.DATEOFOCCURENCE <= #" & Format(DtpEndingDate.Value, "dd/MMM/yyyy") & "# "





            ' strSQL &= "tblSTUDENTSNIDHAMUArchive.DATEOFOCCURENCE, tblSTUDENTSNIDHAMUArchive.OFFENSECATEGORY FROM tblSTUDENTSNIDHAMU INNER JOIN tblSTUDENTSNIDHAMUArchive ON tblSTUDENTSNIDHAMU.ADMISSIONNUMBER = tblSTUDENTSNIDHAMUArchive.ADMISSIONNUMBER WHERE tblSTUDENTSNIDHAMU.DATEOFOCCURENCE >= #" & Format(DtpBeginningDate.Value, "dd/MMM/yyyy") & "# AND tblSTUDENTSNIDHAMU.DATEOFOCCURENCE <= #" & Format(DtpEndingDate.Value, "dd/MMM/yyyy") & "# AND tblSTUDENTSNIDHAMU.ADMISSIONNUMBER = '" & ComboADMNO.Text & "' "




            'strSQL &= "tblSTUDENTSNIDHAMUArchive.DATEOFOCCURENCE, tblSTUDENTSNIDHAMUArchive.OFFENSECATEGORY FROM tblSTUDENTSNIDHAMU INNER JOIN tblSTUDENTSNIDHAMUArchive ON tblSTUDENTSNIDHAMU.ADMISSIONNUMBER = tblSTUDENTSNIDHAMUArchive.ADMISSIONNUMBER WHERE tblSTUDENTSNIDHAMU.DATEOFOCCURENCE >= #" & Format(DtpBeginningDate.Value, "dd/MMM/yyyy") & "# AND tblSTUDENTSNIDHAMU.DATEOFOCCURENCE <= #" & Format(DtpEndingDate.Value, "dd/MMM/yyyy") & "# AND tblSTUDENTSNIDHAMU.ADMISSIONNUMBER = '" & ComboADMNO.Text & "' AND tblSTUDENTSNIDHAMU.ADMISSIONNUMBER IN (SELECT DISTINCT tblNEWSTUDENTS.ADMISSIONNUMBER FROM tblNEWSTUDENTS) AND tblSTUDENTSNIDHAMUArchive.ADMISSIONNUMBER IN (SELECT tblNEWSTUDENTS.ADMISSIONNUMBER FROM tblNEWSTUDENTS) AND tblSTUDENTSNIDHAMUArchive.DATEOFOCCURENCE >= #" & Format(DtpBeginningDate.Value, "dd/MMM/yyyy") & "# AND tblSTUDENTSNIDHAMUArchive.DATEOFOCCURENCE <= #" & Format(DtpEndingDate.Value, "dd/MMM/yyyy") & "#)" 'ORDER BY DATEOFOCCURENCE ASC"




            '  strSQL &= "tblSTUDENTSNIDHAMUArchive.ID, tblSTUDENTSNIDHAMUArchive.DATEOFOCCURENCE, tblSTUDENTSNIDHAMUArchive.ADMISSIONNUMBER, tblSTUDENTSNIDHAMUArchive.TERM, "

            ' strSQL &= "tblSTUDENTSNIDHAMUArchive.NAMEOFSTUDENT, tblSTUDENTSNIDHAMUArchive.GENDER, tblSTUDENTSNIDHAMUArchive.KCPEMARKS, tblSTUDENTSNIDHAMUArchive.KCPEGRADE, tblSTUDENTSNIDHAMUArchive.CURRENTMARKS, tblSTUDENTSNIDHAMUArchive.CURRENTPOINTS, "

            ' strSQL &= "tblSTUDENTSNIDHAMUArchive.CURRENTGRADE, tblSTUDENTSNIDHAMUArchive.STUDENTVAP, tblSTUDENTSNIDHAMUArchive.CLASS, tblSTUDENTSNIDHAMUArchive.STREAM, tblSTUDENTSNIDHAMUArchive.FORMERSCHOOL, tblSTUDENTSNIDHAMUArchive.DATEOFADMISSION, tblSTUDENTSNIDHAMUArchive.DORMITORIES, "

            ' strSQL &= "tblSTUDENTSNIDHAMUArchive.TYPEOFOFFENSE, tblSTUDENTSNIDHAMUArchive.OFFENSECATEGORY, tblSTUDENTSNIDHAMUArchive.FAMILYSITUATION, tblSTUDENTSNIDHAMUArchive.NAMEOFPARENT, tblSTUDENTSNIDHAMUArchive.PARENTPHONENUMBER, tblSTUDENTSNIDHAMUArchive.STUDENTPHOTO, tblSTUDENTSNIDHAMUArchive.COUNTY, "

            ' strSQL &= "tblSTUDENTSNIDHAMUArchive.SUBCOUNTY, tblSTUDENTSNIDHAMUArchive.RESIDENCEESTATE, tblSTUDENTSNIDHAMUArchive.RESPONSIBILITY, tblSTUDENTSNIDHAMUArchive.REMARKS, tblSTUDENTSNIDHAMUArchive.TYPEOFPUNISHMENT, tblSTUDENTSNIDHAMUArchive.FEESBALANCE FROM tblSTUDENTSNIDHAMU INNER JOIN tblSTUDENTSNIDHAMUArchive ON tblSTUDENTSNIDHAMU.ADMISSIONNUMBER = tblSTUDENTSNIDHAMUArchive.ADMISSIONNUMBER WHERE tblSTUDENTSNIDHAMU.DATEOFOCCURENCE >= #" & Format(DtpBeginningDate.Value, "dd/MMM/yyyy") & "# AND tblSTUDENTSNIDHAMU.DATEOFOCCURENCE <= #" & Format(DtpEndingDate.Value, "dd/MMM/yyyy") & "# AND tblSTUDENTSNIDHAMU.ADMISSIONNUMBER = '" & ComboADMNO.Text & "' AND tblSTUDENTSNIDHAMU.ADMISSIONNUMBER IN (SELECT DISTINCT tblNEWSTUDENTS.ADMISSIONNUMBER FROM tblNEWSTUDENTS)" 'ORDER BY DATEOFOCCURENCE ASC"
            ...
            ' strSQL = "Select tblSTUDENTSNIDHAMU.*,
    ...
    Private Sub DtpEndingDate_ValueChanged(sender As Object, e As EventArgs) Handles DtpEndingDate.ValueChanged
        Try
            ...
            'dtadptFillDATEOFOCCURENCE = New OleDb.OleDbDataAdapter("Select * FROM tblSTUDENTSNIDHAMU", conn)



            ' Use the data adapter to fill data in the dataset
            'dtadptFillDATEOFOCCURENCE.Fill(dstFillDATEOFOCCURENCE, "tblSTUDENTSNIDHAMU")
            ...
            'If dstFillDATEOFOCCURENCE.Tables("tblSTUDENTSNIDHAMU").Rows.Count > 0 Then

            'For IntCounter = 0 To dstFillDATEOFOCCURENCE.Tables("tblSTUDENTSNIDHAMU").Rows.Count - 1
            'If dstFillDATEOFOCCURENCE.Tables("tblSTUDENTSNIDHAMU").Rows(IntCounter).Item("OFFENSECATEGORY") = "DISCIPLINED" Then
            'dstFillDATEOFOCCURENCE.Tables("tblSTUDENTSNIDHAMU").Rows(IntCounter).Item("DATEOFOCCURENCE") = DtpEndingDate.Value
            'End If
            'Next

            'dtadptFillDATEOFOCCURENCE.Update(dstFillDATEOFOCCURENCE, "tblSTUDENTSNIDHAMU")
    ...
    Private Sub BtnPendingPunishment_Click(sender As Object, e As EventArgs) Handles BtnPendingPunishment.Click
        Try
            ...
            strSQL &= "TYPEOFOFFENSE, OFFENSECATEGORY, FAMILYSITUATION, NAMEOFPARENT, PARENTPHONENUMBER, TYPEOFPUNISHMENT FROM tblSTUDENTSNIDHAMU WHERE TYPEOFPUNISHMENT = 'Pending' AND DATEOFOCCURENCE >= #" & Format(DtpBeginningDate.Value, "dd/MMM/yyyy") & "# AND DATEOFOCCURENCE <= #" & Format(DtpEndingDate.Value, "dd/MMM/yyyy") & "# AND ACADEMICYEAR = '" & txtCurrentYear.Text & "' AND OFFENSECATEGORY <> 'DISCIPLINED' AND ADMISSIONNUMBER IN (SELECT DISTINCT ADMISSIONNUMBER FROM tblNEWSTUDENTS WHERE DISCONTINUED = FALSE) ORDER BY DATEOFOCCURENCE ASC"
    ...
    Private Sub MajorOffensesPerClass_Click(sender As Object, e As EventArgs) Handles MajorOffensesPerClass.Click
        Try
            ...
            strSQL &= "TYPEOFOFFENSE, OFFENSECATEGORY, FAMILYSITUATION, NAMEOFPARENT,  PARENTPHONENUMBER, TYPEOFPUNISHMENT FROM tblSTUDENTSNIDHAMU WHERE TYPEOFOFFENSE = 'Major' AND DATEOFOCCURENCE >= #" & Format(DtpBeginningDate.Value, "dd/MMM/yyyy") & "# AND DATEOFOCCURENCE <= #" & Format(DtpEndingDate.Value, "dd/MMM/yyyy") & "# AND ACADEMICYEAR = '" & txtCurrentYear.Text & "' AND OFFENSECATEGORY <> 'DISCIPLINED' AND ADMISSIONNUMBER IN (SELECT DISTINCT ADMISSIONNUMBER FROM tblNEWSTUDENTS WHERE DISCONTINUED = FALSE) ORDER BY DATEOFOCCURENCE ASC"
    ...
    Private Sub MinorOffensesPerClass_Click(sender As Object, e As EventArgs) Handles MinorOffensesPerClass.Click
        Try
            ...
            strSQL &= "TYPEOFOFFENSE, OFFENSECATEGORY, FAMILYSITUATION, NAMEOFPARENT,  PARENTPHONENUMBER, TYPEOFPUNISHMENT FROM tblSTUDENTSNIDHAMU WHERE TYPEOFOFFENSE = 'Minor' AND DATEOFOCCURENCE >= #" & Format(DtpBeginningDate.Value, "dd/MMM/yyyy") & "# AND DATEOFOCCURENCE <= #" & Format(DtpEndingDate.Value, "dd/MMM/yyyy") & "# AND ACADEMICYEAR = '" & txtCurrentYear.Text & "' AND OFFENSECATEGORY <> 'DISCIPLINED' AND ADMISSIONNUMBER IN (SELECT DISTINCT ADMISSIONNUMBER FROM tblNEWSTUDENTS WHERE DISCONTINUED = FALSE) ORDER BY DATEOFOCCURENCE ASC"
    ...
    Private Sub BtnMajorOffencesClassStatistics_Click(sender As Object, e As EventArgs) Handles BtnMajorOffencesClassStatistics.Click
        Try
            ...
            strSQL = "SELECT CLASS, STREAM, TYPEOFOFFENSE, OFFENSECATEGORY, COUNT(OFFENSECATEGORY) as OCCURENCECOUNT, COUNT(ADMISSIONNUMBER) as StreamEnrolment From tblSTUDENTSNIDHAMU WHERE TYPEOFOFFENSE = 'Major' AND DATEOFOCCURENCE >= #" & Format(DtpBeginningDate.Value, "dd/MMM/yyyy") & "# AND DATEOFOCCURENCE <= #" & Format(DtpEndingDate.Value, "dd/MMM/yyyy") & "# AND ACADEMICYEAR = '" & txtCurrentYear.Text & "' AND OFFENSECATEGORY <> 'DISCIPLINED' AND ADMISSIONNUMBER IN (SELECT DISTINCT ADMISSIONNUMBER FROM tblNEWSTUDENTS WHERE DISCONTINUED = FALSE) GROUP BY CLASS, STREAM, TYPEOFOFFENSE, OFFENSECATEGORY ORDER BY CLASS ASC"
    ...
    Private Sub BtnMinorOffencesClassStatistics_Click(sender As Object, e As EventArgs) Handles BtnMinorOffencesClassStatistics.Click
        Try
            ...
            strSQL = "SELECT CLASS, STREAM, TYPEOFOFFENSE, OFFENSECATEGORY, COUNT(OFFENSECATEGORY) as OCCURENCECOUNT, COUNT(ADMISSIONNUMBER) as StreamEnrolment From tblSTUDENTSNIDHAMU WHERE TYPEOFOFFENSE = 'Minor' AND DATEOFOCCURENCE >= #" & Format(DtpBeginningDate.Value, "dd/MMM/yyyy") & "# AND DATEOFOCCURENCE <= #" & Format(DtpEndingDate.Value, "dd/MMM/yyyy") & "# AND ACADEMICYEAR = '" & txtCurrentYear.Text & "' AND OFFENSECATEGORY <> 'DISCIPLINED' AND ADMISSIONNUMBER IN (SELECT DISTINCT ADMISSIONNUMBER FROM tblNEWSTUDENTS WHERE DISCONTINUED = FALSE) GROUP BY CLASS, STREAM, TYPEOFOFFENSE, OFFENSECATEGORY ORDER BY CLASS ASC"
    ...
    Private Sub BtnMajorOffencesDormStatistics_Click(sender As Object, e As EventArgs) Handles BtnMajorOffencesDormStatistics.Click
        Try
            ...
            strSQL = "SELECT DORMITORIES, TYPEOFOFFENSE, OFFENSECATEGORY, COUNT(OFFENSECATEGORY) as OCCURENCECOUNT, COUNT(ADMISSIONNUMBER) as DORMENROLMENT From tblSTUDENTSNIDHAMU WHERE TYPEOFOFFENSE = 'Major' AND DATEOFOCCURENCE >= #" & Format(DtpBeginningDate.Value, "dd/MMM/yyyy") & "# AND DATEOFOCCURENCE <= #" & Format(DtpEndingDate.Value, "dd/MMM/yyyy") & "# AND ACADEMICYEAR = '" & txtCurrentYear.Text & "' AND OFFENSECATEGORY <> 'DISCIPLINED' AND ADMISSIONNUMBER IN (SELECT DISTINCT ADMISSIONNUMBER FROM tblNEWSTUDENTS WHERE DISCONTINUED = FALSE) GROUP BY DORMITORIES, TYPEOFOFFENSE, OFFENSECATEGORY ORDER BY DORMITORIES ASC"
    ...
    Private Sub BtnMinorOffencesDormStatistics_Click(sender As Object, e As EventArgs) Handles BtnMinorOffencesDormStatistics.Click
        Try
            ...
            strSQL = "SELECT DORMITORIES, TYPEOFOFFENSE, OFFENSECATEGORY, COUNT(OFFENSECATEGORY) as OCCURENCECOUNT, COUNT(ADMISSIONNUMBER) as DORMENROLMENT From tblSTUDENTSNIDHAMU WHERE TYPEOFOFFENSE = 'Minor' AND DATEOFOCCURENCE >= #" & Format(DtpBeginningDate.Value, "dd/MMM/yyyy") & "# AND DATEOFOCCURENCE <= #" & Format(DtpEndingDate.Value, "dd/MMM/yyyy") & "# AND ACADEMICYEAR = '" & txtCurrentYear.Text & "' AND OFFENSECATEGORY <> 'DISCIPLINED' AND ADMISSIONNUMBER IN (SELECT DISTINCT ADMISSIONNUMBER FROM tblNEWSTUDENTS WHERE DISCONTINUED = FALSE) GROUP BY DORMITORIES, TYPEOFOFFENSE, OFFENSECATEGORY ORDER BY DORMITORIES ASC"
    ...
    Private Sub BtnMajorVsMinorOffencesPercentages_Click(sender As Object, e As EventArgs) Handles BtnMajorVsMinorOffencesPercentages.Click
        Try
            ...
            ' strSQL = "SELECT OFFENSECATEGORY, COUNT(OFFENSECATEGORY) as OCCURENCECOUNT From tblSTUDENTSNIDHAMU WHERE DATEOFOCCURENCE >= #" & Format(DtpBeginningDate.Value, "dd/MMM/yyyy") & "# AND DATEOFOCCURENCE <= #" & Format(DtpEndingDate.Value, "dd/MMM/yyyy") & "# AND OFFENSECATEGORY <> 'DISCIPLINED' AND ADMISSIONNUMBER IN (SELECT DISTINCT ADMISSIONNUMBER FROM tblNEWSTUDENTS WHERE DISCONTINUED = FALSE) GROUP BY OFFENSECATEGORY ORDER BY OFFENSECATEGORY DESC"

            strSQL = "SELECT TYPEOFOFFENSE, COUNT(TYPEOFOFFENSE) as ALLOFFENCESCOUNT From tblSTUDENTSNIDHAMU WHERE DATEOFOCCURENCE >= #" & Format(DtpBeginningDate.Value, "dd/MMM/yyyy") & "# AND DATEOFOCCURENCE <= #" & Format(DtpEndingDate.Value, "dd/MMM/yyyy") & "# AND TYPEOFOFFENSE <> 'NONE' AND ACADEMICYEAR = '" & txtCurrentYear.Text & "' AND OFFENSECATEGORY <> 'DISCIPLINED' AND ADMISSIONNUMBER IN (SELECT DISTINCT ADMISSIONNUMBER FROM tblNEWSTUDENTS WHERE DISCONTINUED = FALSE) GROUP BY TYPEOFOFFENSE ORDER BY TYPEOFOFFENSE ASC"
    ...
    Private Sub BtnAllStudentsDispCard_Click(sender As Object, e As EventArgs) Handles BtnAllStudentsDispCard.Click
        Try
            ...
            strSQL &= "TYPEOFOFFENSE, OFFENSECATEGORY, FAMILYSITUATION, NAMEOFPARENT,  PARENTPHONENUMBER, STUDENTPHOTO, FAMILYSITUATION, COUNTY, SUBCOUNTY, RESIDENCEESTATE, RESPONSIBILITY, REMARKS, TYPEOFPUNISHMENT, FEESBALANCE FROM tblSTUDENTSNIDHAMU WHERE DATEOFOCCURENCE >= #" & Format(DtpBeginningDate.Value, "dd/MMM/yyyy") & "# AND DATEOFOCCURENCE <= #" & Format(DtpEndingDate.Value, "dd/MMM/yyyy") & "# AND ADMISSIONNUMBER IN (SELECT DISTINCT ADMISSIONNUMBER FROM tblNEWSTUDENTS) ORDER BY DATEOFOCCURENCE ASC"
    ...
    Private Sub BtnStudentOffenceNarration_Click(sender As Object, e As EventArgs) Handles BtnStudentOffenceNarration.Click
        Try
            ...
            strSQL &= "TYPEOFOFFENSE, OFFENSECATEGORY, OFFENSECOMMITED, TYPEOFPUNISHMENT FROM tblSTUDENTSNIDHAMU WHERE DATEOFOCCURENCE >= #" & Format(DtpBeginningDate.Value, "dd/MMM/yyyy") & "# AND DATEOFOCCURENCE <= #" & Format(DtpEndingDate.Value, "dd/MMM/yyyy") & "# AND ADMISSIONNUMBER = '" & ComboADMNO.Text & "' AND ADMISSIONNUMBER IN (SELECT DISTINCT ADMISSIONNUMBER FROM tblNEWSTUDENTS) ORDER BY DATEOFOCCURENCE ASC"
    ...
    Private Sub BtnPrintAllYears_Click(sender As Object, e As EventArgs) Handles BtnPrintAllYears.Click
        Try

            frmStatisticalAnalysis2.Show()
    ...
    Private Sub BtnClassEnrolment_Click(sender As Object, e As EventArgs) Handles BtnClassEnrolment.Click
        ...
        Try
            ...
            strSQL = "SELECT CLASS, GENDER, STREAM, COUNT(ADMISSIONNUMBER) as ENROLMENT FROM tblNEWSTUDENTS WHERE DATEOFOCCURENCE >= #" & Format(DtpBeginningDate.Value, "dd/MMM/yyyy") & "# AND DATEOFOCCURENCE <= #" & Format(DtpEndingDate.Value, "dd/MMM/yyyy") & "# AND ACADEMICYEAR = '" & txtCurrentYear.Text & "' AND DISCONTINUED = FALSE GROUP BY CLASS, GENDER, STREAM ORDER BY CLASS ASC"
    ...
    Private Sub BtnIndisciplinedStudentsBySingleClass_Click(sender As Object, e As EventArgs) Handles BtnIndisciplinedStudentsBySingleClass.Click
        ...
        Try
            ...
            strSQL &= "TYPEOFOFFENSE, OFFENSECATEGORY, FAMILYSITUATION, NAMEOFPARENT,  PARENTPHONENUMBER, TYPEOFPUNISHMENT FROM tblSTUDENTSNIDHAMU WHERE DATEOFOCCURENCE >= #" & Format(DtpBeginningDate.Value, "dd/MMM/yyyy") & "# AND DATEOFOCCURENCE <= #" & Format(DtpEndingDate.Value, "dd/MMM/yyyy") & "# AND ACADEMICYEAR = '" & txtCurrentYear.Text & "' AND CLASS = '" & CombClass.Text & "' AND STREAM = '" & CombStream.Text & "' AND OFFENSECATEGORY <> 'DISCIPLINED' AND ADMISSIONNUMBER IN (SELECT DISTINCT ADMISSIONNUMBER FROM tblNEWSTUDENTS WHERE DISCONTINUED = FALSE) ORDER BY DATEOFOCCURENCE ASC"
    ...
    Private Sub BtnClassLists_Click(sender As Object, e As EventArgs) Handles BtnClassLists.Click
        ...
        Try
            ...
            Dim objCmd As OleDbCommand
            Dim objReader As OleDbDataReader
            Dim objDataset As DataSet = New dsClassLists
            Dim strSQL As String
            ...
            strSQL = "Select ADMISSIONNUMBER, NAMEOFSTUDENT, GENDER, CLASS, STREAM, PARENTPHONENUMBER, KCPEMARKS FROM tblSTUDENTSNIDHAMU WHERE DATEOFOCCURENCE >= #" & Format(DtpBeginningDate.Value, "dd/MMM/yyyy") & "# And DATEOFOCCURENCE <= #" & Format(DtpEndingDate.Value, "dd/MMM/yyyy") & "# And ACADEMICYEAR = '" & txtCurrentYear.Text & "' AND CLASS = '" & CombClass2.Text & "' AND STREAM = '" & CombStream2.Text & "' AND ADMISSIONNUMBER IN (SELECT DISTINCT ADMISSIONNUMBER FROM tblNEWSTUDENTS WHERE DISCONTINUED = FALSE) ORDER BY ADMISSIONNUMBER ASC"



            '            strSQL = "Select ADMISSIONNUMBER, NAMEOFSTUDENT, GENDER, CLASS, STREAM, PARENTPHONENUMBER FROM tblNEWSTUDENTS WHERE DATEOFOCCURENCE >= #" & Format(DtpBeginningDate.Value, "dd/MMM/yyyy") & "# AND DATEOFOCCURENCE <= #" & Format(DtpEndingDate.Value, "dd/MMM/yyyy") & "# AND ACADEMICYEAR = '" & txtCurrentYear.Text & "' AND CLASS = '" & CombClass2.Text & "' AND STREAM = '" & CombStream2.Text & "' ORDER BY ADMISSIONNUMBER ASC"
            ...
            Dim rds As ReportDataSource = New ReportDataSource
            ' Note: "dsClassLists_dtClassLists" was name we gave to the report dataset 
            ' when setting it up with the wizard
            rds.Name = "dsClassLists_dtClassLists"
            rds.Value = objDataset.Tables(0)
            Dim paramList As New Generic.List(Of ReportParameter)
            paramList.Add(New ReportParameter("prmBeginningDate", Format(DtpBeginningDate.Value, "dd/MMM/yyyy")))
            paramList.Add(New ReportParameter("prmEndingDate", Format(DtpEndingDate.Value, "dd/MMM/yyyy")))

            With frmClassListsReportviewer
                ' Note: When assigning the ReportEmbeddedResource in the statement below, you must
                ' qualify the report object name with the project name (in this case "WANAFUNZINIDHAMUTRACKER")
                .rvClassListsReportViewer.LocalReport.ReportEmbeddedResource = "WANAFUNZINIDHAMUTRACKER.rptClassLists.rdlc"
                .rvClassListsReportViewer.LocalReport.DataSources.Clear()
                .rvClassListsReportViewer.LocalReport.DataSources.Add(rds)
                .rvClassListsReportViewer.LocalReport.SetParameters(paramList)
                .rvClassListsReportViewer.RefreshReport()
                .Text = "Students Class Lists"
                .ShowDialog()
            End With
